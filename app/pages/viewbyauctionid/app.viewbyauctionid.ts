...
@Component({
    ...
    templateUrl:'app/pages/viewbyauctionid/home.html',
    providers: [AppCommonservices]
    //directives: [FORM_DIRECTIVES, REACTIVE_FORM_DIRECTIVES]
})
export class AppViewbyauctionid implements OnInit, OnDestroy {
    myModal :ModalModule;
    data:any;
    http:Http;
    items:any;
    ...
    constructor(b: FormBuilder , http:Http ,commonservices: AppCommonservices,userInfo:CookieService,router: Router,private route: ActivatedRoute) {
        this.router = router;
        this.http = http;
        this.router = router;
        ...
        this.http.get(link)
            .subscribe(data1 => {
                ...
                var x: any;
                var y: any;
                console.log('auction data with inventory ... ');
                console.log(this.data);
                //let timeoutId = setInterval(() => {
                for (x in this.data) {
                    //console.log('cjecking ids'+this.id+' and '+this.data[x]._id);
                    for (y in this.data[x].cardata) {
                        console.log('in aucton loop');
                        ...
                        console.log(this.data[x].auctiondata);
                        //this.data[x].cardata[y].auctionids=this.data[x].cardata[y].auctionid.join("-");
                        if(this.id==this.data[x].auctiondata[0]._id)this.cardata.push(this.data[x].cardata[y]);
                    }
                }
