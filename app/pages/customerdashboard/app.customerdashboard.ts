...
export class AppCustomerdashboard {
    ...
    private image: any;
    private est_retail_value: any;
    private color: any;
    private maxbidprice: any;
    private pricepercen:any;
    private price:any;
    bigimage:any;
   ...
   imgadd:any;
        ...
        if (this.carlistarr.length == 0) {
            this.http.get(this.serverUrl + 'carlist')
                .subscribe(data1 => {
                    ...
                    for (x in this.carlistarr) {
                        //this.bigimage[x]=this.filesrc + this.carlistarr[x].filename;
            ...
            for (x in this.carlistarr) {
                if (val == this.carlistarr[x]._id) {
                    ...
                    //console.log('bigimage : '+this.bigimage[x]);
                   // console.log('filename : '+this.filesrc + this.carlistarr[x].filename);
    ...
    rsvpcommit(inventory: Array<any>) {
        this.inventoryarr = inventory;
        //console.log('rsvp data');
       // console.log(inventory);
        this.make = this.getcardetails(this.inventoryarr.inventoryid, 'make');
        this.model = this.getcardetails(this.inventoryarr.inventoryid, 'model');
        this.color = this.getcardetails(this.inventoryarr.inventoryid, 'color');
        this.year = this.getcardetails(this.inventoryarr.inventoryid, 'year');
        this.mileage = this.getcardetails(this.inventoryarr.inventoryid, 'mileage');
        this.carbodystylelist = this.getcardetails(this.inventoryarr.inventoryid, 'carbodystylelist');
        this.year = this.getcardetails(this.inventoryarr.inventoryid, 'year');
        this.image = this.getcardetails(this.inventoryarr.inventoryid, 'cariimage');
        this.est_retail_value = this.getcardetails(this.inventoryarr.inventoryid, 'est_retail_value');
        // console.log(this.colorname);
        let items = {
            make: this.make,
            color: this.color,
            bodystyle: this.carbodystylelist,
            model: this.model,
            mileage: this.mileage,
            filename: this.image,
            est_retail_value: this.est_retail_value,
            customeremail: this.userDetails.useremail,
            year: this.year
        };

        let values = {
            id: this.inventoryarr._id,
            customername: this.userDetails.userfullname,
            dealername: this.name,
            dealeremail: this.email,
            carinfo: items
        };
       // console.log(values);
        this.http.post(this.serverUrl + 'customerrsvpsend', values)
            .subscribe(data => {
    ...
    rsvpuncommit(inventory: Array<any>) {
        this.inventoryarr = inventory;
       // console.log('rsvp data');
       // console.log(inventory);
        this.make = this.getcardetails(this.inventoryarr.inventoryid, 'make');
       ...
       // console.log(values);
        this.http.post(this.serverUrl + 'customerrsvpsenduncommit', values)
            .subscribe(data => {
    ...
    getmaxbid(price1: number) {
        this.price=price1;
         this.pricepercen=this.price*5/100;
        let maxbid:number=this.price;
        this.maxbidprice = parseInt(this.price + (this.pricepercen));
        return this.maxbidprice;
    }
    setmaxbid(id:any) {
        let maxbid = $('#setmax'+id).val();
        let valuess = {id: id, maxbid: maxbid};
        // customersetmaxbitall
        //  customersetmaxbit
       // console.log(valuess);
        if (maxbid == '') {
alert(222);
        ...
        this.http.post(this.serverUrl + 'customersetmaxbit', valuess)
            .subscribe(data => {
              //  console.log(555555);
                let linkv1 = this.serverUrl + 'getrsvpbydealeridforuser';
                let var11 = {dealerid: this.dealerid, customerid: this.userDetails.username};
    ...
    customerparticipateCheck(id1:any,customer_participiate:any){
      //  if($('.rsvpinvitationcheck').is(':checked')){
            //console.log(3233);
            let values = {id: id1, customer_participiate: customer_participiate};
        //console.log(values);
            this.http.post(this.serverUrl + 'customerparticipate', values)
                .subscribe(data => {
                    //console.log(4545545455);
                    let linkv1 = this.serverUrl + 'getrsvpbydealeridforuser';
                    let var11 = {dealerid: this.dealerid, customerid: this.userDetails.username};
                    this.http.post(linkv1, var11)
                        .subscribe(data1 => {
    ...
    imageshow(imagesrc:any,imagename:any,ev:any){
        ...
        //$(target).parent().parent().prev().attr('ng-reflect-inner-h-t-m-l','<img src= '+imagesrc+imagename+' />');
        $(target).parent().parent().prev().remove();
        $(target).parent().parent().parent().prepend('<div class="cd_block3_gallery_bigimg imgshow" ><img src="'+imagesrc+imagename+'"></div>');


        //$('.imgshow img').attr('src',imagesrc+imagename);
    }
    //}
    getimg(src2:any){
