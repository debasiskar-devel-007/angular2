...
@Component({
    ...
    templateUrl:'app/pages/editbaseprice/home.html',
    providers: [AppCommonservices]
    //directives: [FORM_DIRECTIVES, REACTIVE_FORM_DIRECTIVES]
})
export class AppEditbaseprice  implements OnInit, OnDestroy {
    ...
    constructor(fb: FormBuilder , http:Http ,commonservices: AppCommonservices ,userInfo:CookieService ,router: Router,private route: ActivatedRoute,appcomponent:AppComponent) {
        ...
        this.sub = this.route.params.subscribe(params => {
            ...
            let ids={id:this.id};
            this.http.post(this.serverUrl+'editbaseprice',ids)
                .subscribe(data => {
                    this.details=data.json()[0];
                    console.log(this.details);
                    this.addadminform = fb.group({
                        baseprice: [this.details.baseprice, Validators.required],
    ...
    submitform(){
        ...
        if(this.addadminform.valid){
            let link = this.serverUrl+'updatebaseprice';
            var submitdata = this.addadminform.value;
            this.http.post(link,submitdata)
                .subscribe(data => {
                    // /this.data1.response = data.json();
                    this.router.navigateByUrl('/basepricelist(adminheader:adminheader//adminfooter:adminfooter)')
