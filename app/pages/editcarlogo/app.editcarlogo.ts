...
@Component({
    ...
    templateUrl:'app/pages/editcarlogo/home.html',
    providers: [AppCommonservices]
    //directives: [FORM_DIRECTIVES, REACTIVE_FORM_DIRECTIVES]
})
export class AppEditcarlogo  implements OnInit, OnDestroy {
    ...
    constructor(fb: FormBuilder , http:Http ,commonservices: AppCommonservices ,userInfo:CookieService ,router: Router,private route: ActivatedRoute) {
        ...
        this.sub = this.route.params.subscribe(params => {
            ...
            let ids={id:this.id};
            this.http.post(this.serverUrl+'getcarlogobyid',ids)
                .subscribe(data => {
   ...
   handleUpload(data: any): void {
    ...
    submitform(){
        ...
        if(this.addadminform.valid){
            let link = this.serverUrl+'updatecarlogo';
            var submitdata = this.addadminform.value;
            this.http.post(link,submitdata)
                .subscribe(data => {
                    // /this.data1.response = data.json();
                    this.router.navigateByUrl('/carlogolist(adminheader:adminheader//adminfooter:adminfooter)')
